...
void BasisExpansion::computeCoefficients(
        double r, vec d, double weight, double weight_scale, 
        double sigma, bool gradients) {
    // SETUP STORAGE
    int L = _angbasis->L();
    int N = _radbasis->N();
    if (!_has_scalars) {
        // Playing it safe ... should have already been done in constructor
        _has_scalars = true;
        _radcoeff = RadialBasis::radcoeff_zero_t(N,L+1);
        _angcoeff = AngularBasis::angcoeff_zero_t((L+1)*(L+1));
        _coeff = coeff_zero_t(N,(L+1)*(L+1));
    ...
    if (_has_scalars && !_has_gradients) {
        ...
        _radbasis->computeCoefficients(d, r, sigma, _radcoeff, 
            &_radcoeff_grad_x, &_radcoeff_grad_y, &_radcoeff_grad_z);
        _angbasis->computeCoefficients(d, r, sigma, _angcoeff, 
            &_angcoeff_grad_x, &_angcoeff_grad_y, &_angcoeff_grad_z);
...
void BasisExpansion::registerPython() {
    ...
    class_<BasisExpansion, BasisExpansion*>("BasisExpansion", init<Basis*>())
        .add_property("array", &BasisExpansion::getCoefficientsNumpy, 
            &BasisExpansion::setCoefficientsNumpy)
